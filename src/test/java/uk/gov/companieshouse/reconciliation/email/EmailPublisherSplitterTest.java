...
public class EmailPublisherSplitterTest {
    ...
    void testReturnListOfPublisherResourceRequests() {
        //given
        PublisherResourceRequestWrapper resourceRequestWrapper = new PublisherResourceRequestWrapper(Collections.singletonList(new PublisherResourceRequest("key", 300, "uploader", "presigner", "description", "BODY".getBytes(), "group", "linkId", true)));

        //when
        List<PublisherResourceRequest> actual = splitter.split(resourceRequestWrapper);

        //then
        assertEquals(Collections.singletonList(new PublisherResourceRequest("key", 300, "uploader", "presigner", "description", "BODY".getBytes(), "group", "linkId", true)), actual);
    }
}
